...
export const meditationService = {
  ...
  getRecommendedMeditation({ duration, guidanceType, preferredMethod, previousPractice }) {
    ...
    return [...pool]
      .map((session) => ({
        session,
        score:
          Math.abs(session.durationSeconds - target) / 60 -
          (session.methodId === preferredMethod ? 3 : 0) -
          (session.methodId === previousMethod ? 2 : 0),
      }))
      .sort((a, b) => a.score - b.score)[0]?.session
  },
  ...
  getRecentPractice() {
    try {
      return JSON.parse(localStorage.getItem(HISTORY_KEY) || '[]')[0] || null
    } catch {
      return null
    }
  },
  getContinuePractice() {
    try {
      ...
      return history.find((item) => item.updatedAt && !item.completed && (item.progressSeconds ?? item.durationCompleted ?? 0) > 0) || null
    } catch {
      return null
    }
  },
  getPreferredGuidance() {
    try {
      const saved = localStorage.getItem(GUIDANCE_KEY)
      return saved === 'silent' ? 'silent' : 'guided'
    } catch { return 'guided' }
  },
  savePreferredGuidance(guidanceType) {
    try { localStorage.setItem(GUIDANCE_KEY, guidanceType) } catch { /* optional preference */ }
  },
  saveProgress(progress) {
    this.savePracticeSession(progress)
  },
  discardPractice(sessionId) {
    try {
      const current = JSON.parse(localStorage.getItem(HISTORY_KEY) || '[]')
      localStorage.setItem(HISTORY_KEY, JSON.stringify(current.filter((item) => (item.sessionId || item.meditationSessionId || item.id) !== sessionId)))
    } catch {
      // Stopping the active timer still works when storage is unavailable.
    }
  },
  discardOtherIncompletePractices(activeSessionId) {
    try {
      ...
      const filtered = current.filter((item) => {
        const itemId = item.sessionId || item.meditationSessionId || item.id
        return item.completed || itemId === activeSessionId
      })
      localStorage.setItem(HISTORY_KEY, JSON.stringify(filtered))
    } catch {
      // Starting a new session still works when storage is unavailable.
    }
  },
  completeSession(progress) {
    this.savePracticeSession({ ...progress, completed: true, completedAt: new Date().toISOString() })
  },
  savePracticeSession(practice) {
    try {
      const current = JSON.parse(localStorage.getItem(HISTORY_KEY) || '[]')
      const practiceKey = practice.sessionId || practice.meditationSessionId || practice.id
      const timestampedPractice = { ...practice, updatedAt: new Date().toISOString() }
      localStorage.setItem(HISTORY_KEY, JSON.stringify([timestampedPractice, ...current.filter((item) => (item.sessionId || item.meditationSessionId || item.id) !== practiceKey)].slice(0, 20)))
    } catch {
