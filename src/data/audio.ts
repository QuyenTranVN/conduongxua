...
const MEDITATION_ATTRIBUTION = 'Âm thanh được chia sẻ với sự cho phép của Thiền Dưỡng Sinh DASIRA NARADA.'
const THERAVADA_SOURCE = { name: 'Theravāda.vn', pageUrl: 'https://theravada.vn' }
const THERAVADA_ATTRIBUTION = 'Âm thanh được chia sẻ với sự cho phép của Theravāda.vn.'
...
const GOENKA_SATIPATTHANA_TALK = [
  ['1', 'tu-niem-xu-giang-giai-phan-01.mp3', 3600],
  ['2', 'tu-niem-xu-giang-giai-phan-02.mp3', 3599.986939],
  ['3', 'tu-niem-xu-giang-giai-phan-03.mp3', 3600.013061],
  ['4', 'tu-niem-xu-giang-giai-phan-04.mp3', 3599.986939],
  ['5', 'tu-niem-xu-giang-giai-phan-05.mp3', 3600.013061],
  ['6', 'tu-niem-xu-giang-giai-phan-06.mp3', 1476.967619],
] as const
...
export const AUDIO_ITEMS = [
  ...DAILY_CHANTING.map((item) => ({
    ...
    attribution: THERAVADA_ATTRIBUTION, permission: { status: 'approved' as const },
  })),
  ...ASHIN_SARANA_SUTTAS.map((item) => ({
    ...
    source: THERAVADA_SOURCE, attribution: THERAVADA_ATTRIBUTION,
    permission: { status: 'approved' as const },
  })),
  ...GOENKA_SATIPATTHANA_TALK.map(([part, file, duration]) => ({
    id: `goenka-tu-niem-xu-giang-giai-${part}`,
    slug: `goenka-tu-niem-xu-giang-giai-phan-${part}`,
    title: `Tứ Niệm Xứ Giảng Giải — Phần ${part}`,
    teacher: 'Thiền sư S.N. Goenka', category: 'dhamma' as const, language: 'vi' as const,
    duration, image: '/images/scenes/forest.jpg', audioPath: goenkaTalkPath(file),
    collection: 'Tứ Niệm Xứ Giảng Giải',
    source: THERAVADA_SOURCE, attribution: THERAVADA_ATTRIBUTION,
    permission: { status: 'approved' as const },
  })),
  {
    ...
    collection: 'Pháp thoại Ajahn Chah', source: THERAVADA_SOURCE, attribution: THERAVADA_ATTRIBUTION, permission: { status: 'approved' as const },
  },
  {
  ...
  ...AJAHN_BRAHM_MEDITATIONS.map(([part, title, file, duration]) => ({
    ...
    source: MEDITATION_SOURCE, attribution: MEDITATION_ATTRIBUTION, permission: { status: 'approved' as const },
  })),
  ...[
    ['8', '8 phut HUONG-DAN-THIEN-CAN-BAN.mp3', 480],
    ['15', '15-phut-Thien.mp3', 900],
    ...
    attribution: MEDITATION_ATTRIBUTION,
    permission: { status: 'approved' as const },
  })),
] satisfies AudioItem[]
