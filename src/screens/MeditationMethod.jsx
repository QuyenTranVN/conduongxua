...
export default function MeditationMethod({ id }) {
  ...
  return <><AppBar title={method.name} /><div className='scroll has-mini method-detail'>
    ...
    <div className='sec'>Thực hành nhanh</div><QuickPracticeDurationSelector durations={[8, 15, 30, 60, 90, 120]} onSelect={setQuickMinutes} />
    <div className='sec'>Các buổi thực hành</div><div className='seg method-filters'>{(lang === 'en' ? [['all', 'All'], ['guided', 'Guided'], ['silent', 'Silent']] : [['all', 'Tất cả'], ['guided', 'Có hướng dẫn'], ['silent', 'Im lặng']]).map(([value, label]) => <button key={value} aria-pressed={filter === value} onClick={() => setFilter(value)}>{label}</button>)}</div>
    <div className='card' style={{ marginTop: 10 }}>{shown.map((session) => { const teacher = teacherById(session.teacherId); return <button className='row session-row' key={session.id} onClick={() => start(session)}><span className='session-row__play'><Icon name='play' size={15} fill /></span><span className='grow'><span className='tl' style={{ display: 'block' }}>{session.titleVi}</span><span className='tc'>{teacher?.name || session.teacherName || 'Tự thực hành'} · {GUIDANCE_LABELS[session.guidanceType]}</span></span><span className='tm'>{session.durationSeconds / 60} phút</span></button> })}</div>
    <div className='sec'>Tìm hiểu thêm</div><div className='card'>{method.referencesVi.map((reference) => <button className='row' key={reference}><Icon name='book' size={18} /><span className='grow tl'>{reference}</span><Icon name='chev' size={17} /></button>)}</div>
