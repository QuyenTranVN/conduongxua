  ...
  const labels =
    lang === 'vi'
      ? {
          ...
          clear: 'Xóa dữ liệu trên thiết bị',
          confirm: 'Xóa tiến trình, nội dung đã lưu và các tùy chọn trên thiết bị này?',
        }
      : {
          settings: 'Settings',
          about: 'About',
          ...
          confirm: 'Clear progress, saved content, and preferences on this device?',
        }
  const open = (route) => {
    setDrawer(false)
    go(route)
  ...
  return (
    <>
      ...
      >
        ...
        <button className='row' style={{ padding: '12px 0' }} onClick={() => open('about')}>
          <Icon name='note' size={20} />
          <span className='grow tl'>{labels.about}</span>
          <Icon name='chev' size={17} />
        </button>
        <button className='row' style={{ padding: '12px 0' }} onClick={() => open('contact')}>
          <Icon name='mail' size={20} />
          <span className='grow tl'>{labels.contact}</span>
          <Icon name='chev' size={17} />
        </button>
        <button className='row' style={{ padding: '12px 0' }} onClick={() => open('privacy')}>
          <Icon name='note' size={20} />
          <span className='grow tl'>{labels.privacy}</span>
          <Icon name='chev' size={17} />
        </button>
        <div className='divider' />
        <button className='row' style={{ padding: '12px 0' }} onClick={clearData}>
          <Icon name='close' size={20} />
          <span className='grow tl'>{labels.clear}</span>
        </button>
        {/* <svg
          className='lotus'
          viewBox='0 0 24 24'
